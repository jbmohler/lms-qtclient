...
class BasicTypePlugin:
    def polish(self, attr, type_, meta):
        if type_ != None:
            ...
            if type_ == 'stringlist':
                f = lambda v: '; '.join([xx for xx in v if xx != None]) if v != None else ''
                meta['formatter'] = formatters.as_xlsx(f)(f)  # apply decorator in ugly way
            if type_ == 'integer':
            ...
            if type_ == 'numeric':
                ...
                meta['formatter'] = lambda value, decimals=decimals: formatters.fixedpoint_nan_formatter(value, decimals)
            ...
            if type_ == 'datetimeflex':
                meta['formatter'] = lambda v: formatters.datetime_formatter(v) if isinstance(v, datetime.datetime) else formatters.date_formatter(v)
                meta['char_width'] = 16
            if type_ == 'currency_usd':
                ...
                if kw.get('blankzero', False):
                    meta['formatter'] = lambda v: formatters.dollar_formatter(v, blankzero=True)
                else:
