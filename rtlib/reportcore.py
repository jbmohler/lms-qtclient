...
class PromptList:
    def __init__(self, column_list=None):
        ...
        values = [(None if x[1] == None else x[1].pop('default', None)) for x in column_list]
        optional_attrs = [x[0] for x in column_list if (False if x[1] == None else x[1].pop('optional', False))]
        relevance_groups = {x[0]: x[1].pop('relevance') for x in column_list if (x[1] != None and 'relevance' in x[1])}
