...
class Exporter(QtWidgets.QDialog):
    ...
    def refresh_data(self):
        ...
        self.backgrounder(self.load_rpt1, self.client.get, self.RPT1_BALANCE_SHEET, date=defdate)
        self.backgrounder(self.load_rpt2, self.client.get, self.RPT2_DETAIL_PL, date1=monthbegin, date2=defdate)
        self.backgrounder(self.load_rpt3, self.client.get, self.RPT3_INTERVAL_PL, ending_date=defdate, intervals=3, length=6)
        self.backgrounder(self.load_rpt4, self.client.get, self.RPT4_GIVING_SUMMARY, date1=yearbegin, date2=defdate, account=offering.main_table().rows[0].id)
