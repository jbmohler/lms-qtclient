...
ColumnDefn = rtlib.fixedrecord(
    "ColumnDefn", ["logical", "attr", "label"], mixin=ColumnDefnMixin
)
...
class ModelColumnChooser(QtWidgets.QDialog):
    ...
    def set_view(self, view):
        ...
        CD = lambda index, c: ColumnDefn(
            logical=index, attr=c.attr, label=c.label.replace("\n", " ")
        )
    ...
    def column_move(self, attr, old_visual, new_visual):
        if old_visual < new_visual:
            ...
            x = [
                h.visualIndex(index)
                for index in range(self._view.model().columnCount(QtCore.QModelIndex()))
            ]
            new_visual = max(x)
