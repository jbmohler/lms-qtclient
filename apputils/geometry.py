...
def get_font_multiplier():
    ...
    if GLOBAL_FONT_MULTIPLIER == None:
        ...
        if platform.system() == 'Windows':
            ...
            GLOBAL_FONT_MULTIPLIER = float(gdi32.GetDeviceCaps(h, LOGPIXELSX))/96.
        else:
            GLOBAL_FONT_MULTIPLIER = 1.
    return GLOBAL_FONT_MULTIPLIER

def get_char_width():
    return int(9.*get_font_multiplier())

def write_grid_geometry(grid, name):
    model = grid.model()
    settings = QtCore.QSettings()
    settings.beginGroup(name)
    header = grid.horizontalHeader() if hasattr(grid, 'horizontalHeader') else grid.header()
    if isinstance(model, models.ObjectQtModel):
        ...
        if grid.isSortingEnabled():
            ...
            order = 'asc' if header.sortIndicatorOrder() == QtCore.Qt.AscendingOrder else 'desc'
            settings.setValue('sort-column', json.dumps([model.columns[section].attr, order]))
...
def read_grid_geometry(grid, name):
    ...
    settings = QtCore.QSettings()
    settings.beginGroup(name)
    total = sum([grid.columnWidth(i) for i in range(model.columnCount(QtCore.QModelIndex()))])
    defaults = None
    defaultTotal = None
    ...
    if 'Column00' in settings.childKeys():
        ...
        if grid.isSortingEnabled():
            ...
            if p != None:
                ...
                if attr in colmap:
                    i = colmap[attr]
                    order = QtCore.Qt.AscendingOrder if direction == 'asc' else QtCore.Qt.DescendingOrder
            grid.sortByColumn(i, order)
...
class WindowGeometry(QtCore.QObject):
    ...
    * widget.objectName()  (recommended)
    ...
    def __init__(self, widget, name=None, size=True, position=True, splitters=None, tabs=None, grids=None):
        ...
        for t_index in range(len(self.tabs)):
            ...
            self.tabs[t_index].actSaveLast.toggled.connect(lambda checked, kind=self.TAB_REOPEN_LAST, x=t_index: self.updateTabLastSave(x, checked, kind))
            self.tabs[t_index].actSaveThis.toggled.connect(lambda checked, kind=self.TAB_REOPEN_SPECIFIED, x=t_index: self.updateTabLastSave(x, checked, kind))
