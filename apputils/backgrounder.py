...
class AnimateWait(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        radfac = (.9-.1)*math.atan(self.clock/100)/math.pi*2
        radius = int(float(min(x.width(), x.height())) * radfac * .5)
        center = x.center()
        arc_rect = QtCore.QRect(center.x() - radius, center.y() - radius, 2*radius, 2*radius)

        c = QtGui.QColor('blue')
        c.setAlpha(40)
        arc_brush = QtGui.QBrush(c)
        for factor in range(7):
            p.setPen(QtGui.QPen(arc_brush, radius // (5+factor), QtCore.Qt.SolidLine, QtCore.Qt.RoundCap))
...
class Backgrounder(QtCore.QObject):
    ...
    def delayed(self, delay, callbacks, call, *args, **kwargs):
        g = callbacks()
        cb2 = next(g)
        delaystruct = {'gen': g, 'status_callbacks': cb2, 'call': call, 'args': args, 'kwargs': kwargs}
    ...
    def check_futures(self):
        ...
        for d in done:
            ...
            try:
                ...
                if d._status_callbacks != None and hasattr(d._status_callbacks, 'terminated'):
                    d._status_callbacks.terminated(exception)
                if exception == None and d._status_callbacks != None and hasattr(d._status_callbacks, 'complete'):
        ...
        for d in not_done:
            if d._status_callbacks != None and hasattr(d._status_callbacks, 'continuing'):
