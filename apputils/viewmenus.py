...
def model_columns_json(model, selected):
    if len(selected):
        sortable_index_list = [(i.column(), i.data(models.ColumnMetaRole)) for i in selected]
...
def model_html_copy(model, selected):
    ...
    rows = []
    if len(selected):
        sortable_index_list = [(index_row_lineage(i), i.data(models.ObjectRole), i.column(), i.data(models.ColumnMetaRole), i) for i in selected]
...
def model_tsv_copy(model, selected):
    ...
    if len(selected):
        ...
        headers = [model.headerData(x, QtCore.Qt.Horizontal, QtCore.Qt.DisplayRole) for x in cols]
        headers = [h.replace('\n', ' ') for h in headers]
...
class Importer(QtWidgets.QDialog):
    ...
    def receiving_columns(self):
        req = [] if self.importer.required_columns == None else self.importer.required_columns
    ...
    def auto_map_columns(self):
        ...
        self.model.headerDataChanged.emit(QtCore.Qt.Horizontal, 0, len(self.raw_headline))
    ...
    def parametrized_imports(self):
        ...
        m = {col.attr: col.import_attr for idx, col in enumerate(self.model.columns) if col.attr != 'status' and col.import_attr != None}
...
class ContextMenu(QtCore.QObject):
    ...
    def __init__(self, view, parent, no_shortcut=None):
        ...
        self.action_copy_selection_formatted = QtWidgets.QAction('Copy &Formatted', self)
        if 'copy' not in no_shortcut:
            self.action_copy_selection_formatted.setShortcut(QtGui.QKeySequence.Copy)
            self.action_copy_selection_formatted.setShortcutContext(QtCore.Qt.WidgetShortcut)
        ...
        self.action_copy_selection_unformatted = QtWidgets.QAction('Copy &Unformatted', self)
        self.action_copy_selection_unformatted.triggered.connect(self.copy_selected_cells_unformatted)
    ...
    def advance_highlight(self):
        ...
        if 0 <= row < m.rowCount(None):
            ...
            self._selmodel.setCurrentIndex(rowpair[0], QtCore.QItemSelectionModel.SelectCurrent)
            self._selmodel.select(itemsel, QtCore.QItemSelectionModel.SelectCurrent)

    def selection_changed(self, selected, deselected):
        values = [index.data(models.DecimalRole) for index in self._selmodel.selectedIndexes()]
    ...
    def copy_selected_cells(self, with_html=True):
        ...
        if len(self.selected_indexes()) == 1:
            ...
            if with_html:
                ...
                m.setData('text/html', html)
            m.setData(COLUMN_MIME_TYPE, model_columns_json(self._view.model(), self.selected_indexes()))
