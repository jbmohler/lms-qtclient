...
def get_save_filename(parent, title, filter, dirname=None):
    if QtCore.__name__.split('.')[0] == 'PySide':
        filename, filter = QtWidgets.QFileDialog.getSaveFileName(parent, title, filter=filter, dir=dirname)
    else:
        filename = QtWidgets.QFileDialog.getSaveFileName(parent, title, filter=filter, dir=dirname)
    if filename == '':
        return None
...
def get_open_filename(parent, title, filter, dirname=None):
    if QtCore.__name__.split('.')[0] == 'PySide':
        filename, filter = QtWidgets.QFileDialog.getOpenFileName(parent, title, filter=filter, dir=dirname)
    else:
        filename = QtWidgets.QFileDialog.getOpenFileName(parent, title, filter=filter, dir=dirname)
    if filename == '':
        return None
    filename = filename.replace('/', os.path.sep)
    return filename
