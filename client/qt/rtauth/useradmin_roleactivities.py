...
class RoleActivityMapperTargets(QtWidgets.QMainWindow):
    ...
    def load(self):
        ...
        try:
            ...
            self.MyRoleActivityMapperRowMixin = type(
                "MyRoleActivityMapperRowMixin", (RoleActivityMapperRowMixin,), fred
            )

            with self.tracker.loading():
                self.records = content.main_table(
                    mixin=self.MyRoleActivityMapperRowMixin
                )
            ...
            columns = [
                c for c in self.records.columns if c.attr in ["act_name", "description"]
            ]
    ...
    def save_targets(self):
        try:
            ...
            files = {
                "roleactivities": tosend.as_http_post_file(
                    exclusions=["act_name", "description"]
                )
            }
