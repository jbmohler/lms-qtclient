...
class ReportPreview(QtWidgets.QWidget):
    def __init__(self, report, session, exports_dir, manager=None):
        ...
        for prompt in self.report.prompts:
            ...
            if prompt.attr in self.report.prompts.relevance_groups:
                ...
                if rtuple[1] == "relevant-if-not":
                    ...
                    self.bound_prompts[rtuple[0]][1].valueChanged.connect(
                        lambda w1=w1, w2=w2, neg_value=rtuple[
                            2
                        ]: update_relevant_enabled(w1, w2, neg_value)
                    )
                    ...
                    if prompt.label.startswith("Begin ") and endprompt.label.startswith(
                        "End "
                    ):
                        prompt.label = prompt.label[6:]
                    elif prompt.label.startswith(
                        "First "
                    ) and endprompt.label.startswith("Last "):
                        prompt.label = prompt.label[6:]
                    ...
                    raise NotImplementedError(
                        f"relevancy method {rtuple[1]} not known."
                    )
            else:
        ...
        self.load_values(
            {p.attr: v for p, v in zip(self.report.prompts, self.report.defaults)}
        )
        ...
        if self.client.session.authorized("put_api_activities"):
            self.power_menu.addAction("&Edit Note").triggered.connect(
                self.cmd_edit_activity
            )
    ...
    def run_wrapper(self):
        ...
        try:
            content = yield apputils.JointBackgrounder(
                apputils.AnimateWait(self.grid), self.report_request_waiter()
            )
        except:
            utils.exception_message(
                self.window(),
                f"The report {self.report.description} generated a server error.",
            )
            return
        finally:
    ...
    def _prepare_current_run(self):
        ...
        if len(self.stack) == 1:
            ...
            self.model = models.TieredObjectQtModel(
                rtlib.parse_columns(exp.aggregate), classes, descendant_attr="_children"
            )
            ...
            for index in range(len(self.expansions)):
                ...
                inners = {
                    ...
                    for key, grp in itertools.groupby(
                        self.stack[index + 1].rows, key=keys[index]
                    )
                }
        ...
        self.my_actions = gridmgr.apply_client_relateds(
            self.power_menu, self.run.content
        )
    ...
    def update_report_line_selection(self, current, previous):
        if self.sidebar != None and hasattr(self.sidebar, "highlight"):
            ...
            if row != None:
                ...
                params = {
                    k: getattr(row, v)
                    for k, v in self.report.sidebars[0]["on_highlight_row"].items()
                }
...
class ReportsManager(QtCore.QObject):
    ...
    def adopt_preview(self, report, w):
        ...
        index = self.main_window.addWorkspaceWindow(
            w, report.description.replace("&", "&&"), fixedpos=True
        )
        # self.tabs.setCurrentIndex(index)
        if (
            currentWidget != None
            and isinstance(currentWidget, ReportPreview)
            and not currentWidget.needs_explicit_close
        ):
