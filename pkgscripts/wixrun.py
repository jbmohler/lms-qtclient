...
class Component:
    ...
    def compid(self):
        if not hasattr(self, "_compid"):
            self._compid = shortened_to_72(
                "comp_" + re.sub(r"[\\.-]", "_", self.source)
            )
        return self._compid
    ...
    def fileid(self):
        if not hasattr(self, "_fileid"):
            self._fileid = shortened_to_72(
                "file_" + re.sub(r"[\\.-]", "_", self.source)
            )
        return self._fileid
...
def run_wix():
    ...
    msiname = os.path.join(
        BASE_DIR, "build", "lmsSuite-{0.ORGNAME}-{0.VERSION}.msi".format(LMS)
    )
