...
def main():
    ...
    rezipset = ['apputils', 'comtypes', 'ewtools', 'valix', 'rtlib', 'fidocore', 'fidolib', 'purchasing', 'scanners']
    ...
    shutil.copyfile(os.path.join(root, 'pkgscripts', 'lms_basic.ico'), os.path.join(outroot, 'binary', 'lms_basic.ico'))
    shutil.copyfile(os.path.join(root, 'pkgscripts', 'assets', 'lms_basic.png'), os.path.join(outroot, 'binary', 'lms_basic.png'))

    # scrubbing PySide2
    PySideDeletes = ['examples', 'glue', 'include', 'qml', 'scripts', 'support', 'translations']
    ...
    web2 += glob.glob(os.path.join(outroot, 'binary', 'lib', 'PySide2', '*WebChannel*.*'))
